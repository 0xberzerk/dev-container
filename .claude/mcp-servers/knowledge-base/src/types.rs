...
pub enum EntrySource {
    ...
    Seed,
    /// Added manually during an audit.
    Manual,
}
...
/// One file per unique query fingerprint in `KnowledgeBase/raw/`.
#[derive(Debug, Clone, Serialize, Deserialize)]
...
/// A seed file dropped by the auditor in `KnowledgeBase/seeds/`.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct SeedFile {
    pub domain: String,
...
// ---------------------------------------------------------------------------
// Curation context (for future scoring — Task 7)
// ---------------------------------------------------------------------------

/// Context provided by the Architect for relevance scoring.
/// Currently unused — scoring uses impact + quality + curation status only.
/// Will be populated when the Architect agent exists (Task 7).
#[derive(Debug, Clone, Default)]
pub struct CurationContext {
    /// Contract names, function names, identifiers from the source.
    pub codebase_keywords: Vec<String>,
    /// From @audit-integration tags.
    pub integration_types: Vec<String>,
    /// From @audit tags or Architect detection.
    pub protocol_categories: Vec<String>,
}
